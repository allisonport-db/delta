...
import io.delta.kernel.types.{ByteType, FieldMetadata, IntegerType, ShortType, StringType, StructType, TimestampNTZType, TimestampType, VariantType}
...
class IcebergWriterCompatV1Suite extends DeltaTableWriteSuiteBase with ColumnMappingSuiteBase {
  ...
  def testIncompatibleTableFeature(
    ...
    if (testOnExistingTable) {
      test(
        s"Cannot enable feature $featureName on an existing table with icebergWriterCompatV1 enabled") {
        withTempDirAndEngine { (tablePath, engine) =>
  ...
  // byte and short blocked PLUS all types blocked by icebergCompatV2

  // ALL THE CHECKS DONE BY ICEBERG COMPAT V2
  // Test any/all enforcement done by IcebergCompatV2 (including type widening checks??)

  // No metadata committed when nothing has changed (i.e. follow-up txn?)

  // Check Nick's changes to see if missing anything important?
}
