...
class IcebergWriterCompatV1Suite extends DeltaTableWriteSuiteBase with ColumnMappingSuiteBase {
  ...
  // All types blocked by icebergCompatV2
  // All checks done by IcebergCompatV2?!! (including type widening checks?)

  // No metadata committed when nothing has changed (i.e. follow-up txn?)

  // Check Spark changes to see if missing anything important?
}
